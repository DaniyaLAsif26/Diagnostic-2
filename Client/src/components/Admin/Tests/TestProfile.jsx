...
// Fields look like plain text while disabled, like inputs while editing
...
// Placeholder data - replace with the test loaded from the API
const test = {
    ...
    category: 'LABORATORY',
    isPopular: true,
    preparation: 'Overnight fasting of 10-12 hours.',
    relevance: ['Diabetes', 'Blood sugar monitoring'],
}
    ...
    return (
        <div className="p-6 lg:p-8">
            ...
            <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <button onClick={() => navigate('/admin/tests')} className="flex items-center gap-1 text-sm font-medium text-slate-500 transition hover:text-brand">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        <span className="rounded-full bg-brand-light/10 px-2.5 py-1 text-xs font-medium text-brand">{test.category}</span>
                ...
                <div className="flex gap-3">
                    {editing && (
                        <button onClick={() => setEditing(false)} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100">
                            Cancel
                        </button>
                    )}
                    <button
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
                <Card
                    ...
                    <div>
                        ...
                        <input type="text" defaultValue={data?.name} disabled={!editing} className={field} />
                    </div>
                    ...
                    <div className="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="number" defaultValue={data?.price} disabled={!editing} className={`${field} pl-12`} />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-sm text-slate-400">Rs.</span>
                                <input type="number" defaultValue={data?.offerPrice} disabled={!editing} className={`${field} pl-12`} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select defaultValue={data?.category} disabled={!editing} className={field}>
                    ...
                    <label className="relative flex items-center justify-between gap-3 rounded-xl border border-slate-200 p-3.5 transition has-[:enabled]:cursor-pointer has-[:enabled]:hover:bg-slate-50">
                        ...
                        <input type="checkbox" defaultChecked={test.isPopular} disabled={!editing} className="peer sr-only" />
                ...
                <Card
                    ...
                    <div>
                        ...
                        <textarea rows={4} defaultValue={test.preparation} disabled={!editing} className={field} />
                    </div>
